...
EMBEDDED_LOGOS = {
    ...
    "all": "<svg xmlns='http://w3.org' viewBox='0 0 160 50' style='width:100%; height:100%;'><rect width='100%' height='100%' fill='#ffffff' rx='6' stroke='#cf142b' stroke-width='2'/><text x='50%' y='55%' font-family='sans-serif' font-weight='bold' font-size='14' fill='#00247d' text-anchor='middle' dominant-baseline='middle'>ALL BRITISH</text></svg>",
...
def build_casino_cards(json_path):
 ...
 \        for key, svg_code in EMBEDDED_LOGOS.items():
        ...
        # בדיקה ישירה ועוקפת: אם זה המותג הבריטי, תזריק לו את העיצוב באופן מיידי וחסין
        if not logo_html and "british" in str(casino.get("brand_name", "")).lower():
